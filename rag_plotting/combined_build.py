...
from rag_plotting.schemas import CombinedSpec, DataSpec, DataColumn, ChartSpec, QueryPlan
from rag_plotting.prompts import build_combined_prompt, detect_requested_chart_type
from rag_plotting.config import MODEL_CONTEXT_WINDOW_TOKENS, RESP_TOKENS_COMBINED, SAFETY_MARGIN_TOKENS
...
def _select_context_under_budget(context_struct: List[Dict[str, Any]], base_prompt_len_tokens: int, response_budget_tokens: int) -> List[Dict[str, Any]]:
...
# ---------- fallbacks ----------
def _build_fallback_markets(context_struct: List[Dict[str, Any]], region_hint: Optional[str]) -> Optional[CombinedSpec]:
    rows = []
    have_numeric = False
    for r in context_struct:
        ...
        if cap is not None and str(cap).strip() != "":
            try:
                num = float(cap); have_numeric = True
            except Exception:
                num = None
    ...
    if not rows:
        return None
    data_spec = DataSpec(
        columns=[DataColumn(name="market", type="string"), DataColumn(name="market_cap", type="number"), DataColumn(name="country", type="string")],
        rows=rows, notes="Fallback CombinedSpec from markets (presence=1 when missing)."
    )
    chart = ChartSpec(chart_type="bar", title=f"Market Cap by Stock Market — {region_hint or 'Selected'}",
                      encodings=_clean_encodings({"x":"market","y":"market_cap"}))
    return CombinedSpec(data_spec=data_spec, chart_spec=chart)

def _build_fallback_pie_markets(context_struct: List[Dict[str, Any]], region_hint: Optional[str]) -> Optional[CombinedSpec]:
    """
    Fallback pie para mercados: usa market_cap si existe; si no, presencia=1.
    """
    rows = []
    have_any = False
    for r in context_struct:
        ...
        if cap is not None and str(cap).strip() != "":
            try:
                ...
                val = None
        rows.append({"market": mk, "share_value": val if val is not None else 1})
        have_any = True
    if not have_any:
        return None
    data_spec = DataSpec(
        columns=[DataColumn(name="market", type="string"), DataColumn(name="share_value", type="number")],
        rows=rows, notes="Fallback PIE — value=market_cap or presence=1"
    )
    chart = ChartSpec(
        chart_type="pie",
        title=f"Market Cap Share by Stock Market — {region_hint or 'Selected'}",
        encodings=_clean_encodings({"label": "market", "value": "share_value"})
    )
    return CombinedSpec(data_spec=data_spec, chart_spec=chart)
...
def _build_fallback_fitch_or_country(user_query: str, context_struct: List[Dict[str, Any]], region_hint: Optional[str], plan: QueryPlan) -> CombinedSpec:
    rows = []
    for r in context_struct:
        country = r.get("country") or r.get("entity")
        if not country: continue
        ...
        rows.append({"country": country, "iso3": iso3, "fitch_rating": fitch, "rating_score": score})
    best_by_country: Dict[str, Dict[str, Any]] = {}
    for rr in rows:
        c = rr["country"]; prev = best_by_country.get(c)
        if prev is None: best_by_country[c] = rr
        else:
    ...
    if chart_type == "bar":
        ...
        enc = _clean_encodings({"location": "iso3" if any((r.get('iso3') or "") for r in rows) else "country","value":"rating_score"})
    chart = ChartSpec(chart_type=chart_type, title=f"{'Bar' if chart_type == 'bar' else 'Map'} — {region_hint or (plan.region or 'Selected')}",
                      region_hint=region_hint, color_legend="Rating score" if chart_type != "bar" else None, encodings=enc)
...
# ---------- main ----------
def call_model_for_combined_spec(user_query: str, context_struct: List[Dict[str, Any]], chart_hint: Optional[str], plan: Optional[QueryPlan] = None) -> CombinedSpec:
    ...
    base_messages = build_combined_prompt(user_query=user_query, context_jsonl="", chart_hint=chart_hint, enforced_chart_type=enforced)
    ...
    messages = build_combined_prompt(user_query=user_query, context_jsonl=context_jsonl, chart_hint=chart_hint, enforced_chart_type=enforced)
    ...
    try:
        ...
        # Si el usuario pidió PIE, intentamos fallback pie primero
        if enforced == "pie" or chart_hint == "pie":
            fb = _build_fallback_pie_markets(context_struct, region_hint)
            if fb:
                fb.chart_spec.encodings = _clean_encodings(fb.chart_spec.encodings)
                print(f"[LLM][Fallback-PIE] rows={len(fb.data_spec.rows)} chart={fb.chart_spec.chart_type}")
                return fb

...
from rag_plotting.aliases_filters import _region_filter, normalize_region
...
def _derive_knobs_from_hints(hints, enumerated_count: int):
    ...
    if hints:
        ...
        if getattr(hints, "k_each", None): base["k_each"] = hints.k_each
        if getattr(hints, "k_total", None): base["k_total"] = hints.k_total
        if getattr(hints, "max_context", None): base["max_context"] = hints.max_context
        if getattr(hints, "max_entities", None): base["max_entities"] = hints.max_entities
        if getattr(hints, "max_subqueries", None): base["max_subq"] = hints.max_subqueries
    ...
    return base["k_each"], base["k_total"], base["max_context"], base["max_entities"], base["max_syn"], base["max_constr"], base["max_subq"]


@ray.remote(num_cpus=0.25, max_retries=1)
def _search_one_remote(collection: str, query: str, k_each: int, region: Optional[str]) -> List[Dict[str, Any]]:
    """
    Ray task: runs filtered + unfiltered searches and dedupes.
    """
    try:
        region_f = _region_filter(region) if region else None
        rows_all: List[Dict[str, Any]] = []

        if FEATURE_HYBRID_FILTERED and region_f:
            try:
                rows_all.extend(search_qdrant_with_query(collection, query, k=k_each, qfilter=region_f))
            except Exception as e:
                rows_all.append({"__error__": f"filtered:{type(e).__name__}:{e}"})
        try:
            rows_all.extend(search_qdrant_with_query(collection, query, k=k_each, qfilter=None))
        except Exception as e:
...
def _multi_search_qdrant(collection: str, queries: List[str], k_each: int, k_total: int, plan: Optional[QueryPlan] = None) -> List[Dict[str, Any]]:
    """Fan-out over many queries, filtered + unfiltered, then fuse with RRF + lexical bump."""
    ...
    for i in range(0, len(dedup), MAX_PARALLEL):
        ...
        jobs = [_search_one_remote.remote(collection, q, k_each, plan.region if plan else None) for q in batch]
...
def _run_once(user_query: str, widen: bool = False):
    ...
    # 3) Knobs
    k_each, k_total, max_ctx, max_entities, max_syn, max_constr, max_subq = _derive_knobs_from_hints(plan.retrieval_hints, len(enumerated.entities))
    if widen:
        k_each = int(k_each * 2.0); k_total = int(k_total * 2.0); max_ctx = int(k_each * 1.5 + max_ctx * 0.5)
    ...
    # 5) Retrieve (HYBRID parallel with Ray)
    ...
    # 6) CombinedSpec (robust parse + fallback)
...
def answer_and_plot(user_query: str):
    """
    Public entrypoint:
      - Plans the query (region-aware, supports compare queries)
      - Expands retrieval across entities/synonyms/constraints
      - Runs HYBRID retrieval (vector + payload filters) and fuses results with lexical bump
      - Asks the reasoning model to produce a CombinedSpec (data + chart), or builds a fallback spec
      - Renders the chart
      Hard guarantee:
      - If the user asked for a BAR CHART, the final chart is BAR (no silent map coercion).
    """
    print(f"\n[Main] === Start ===\nQuery: {user_query}\nMode: {CURRENT_MODE}\n")
    ...
    from rag_plotting.prompts import detect_requested_chart_type  # local import avoids cycles
    if detect_requested_chart_type(user_query) == "bar" and chart.chart_type != "bar":
        ...
        # Pick first string and number columns
        str_cols = [c for c in df.columns if df[c].dtype == "string"]
        num_cols = [c for c in df.columns if pd.api.types.is_numeric_dtype(df[c])] if "pd" in globals() else []
